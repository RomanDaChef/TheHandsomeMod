...
import net.mcreator.handsomemod.gui.RucksackGui;
...
public class BackpackItem extends TheHandsomeModModElements.ModElement {
	@ObjectHolder("the_handsome_mod:backpack")
	public static final Item block = null;
	public BackpackItem(TheHandsomeModModElements instance) {
		super(instance, 77);
	...
	public void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == block) {
			if (Minecraft.getInstance().currentScreen instanceof RucksackGui.GuiWindow) {
	...
	public static class ItemCustom extends Item {
		public ItemCustom() {
			super(new Item.Properties().group(HandsomeMiscItemGroup.tab).maxStackSize(1).rarity(Rarity.COMMON));
			...
			if (entity instanceof ServerPlayerEntity) {
				NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
					...
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						...
						return new RucksackGui.GuiContainerMod(id, inventory, packetBuffer);
					}
				}, buf -> {
